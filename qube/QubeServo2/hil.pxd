...
cdef extern from "/opt/quanser/hil_sdk/include/hil.h":
    ...
    t_error hil_set_encoder_counts(t_card card, const t_uint32 encoder_channels[], t_uint32 num_channels, const t_int32 buffer[])
    t_error hil_write_analog(t_card card, const t_uint32 analog_channels[], t_uint32 num_channels, const t_double buffer[])
    t_error hil_write_digital(t_card card, const t_uint32 digital_lines[], t_uint32 num_lines, const t_boolean buffer[])
    t_error hil_write_other(t_card card, const t_uint32 other_channels[], t_uint32 num_channels, const t_double buffer[])

    t_error hil_read_analog(t_card card, const t_uint32 analog_channels[], t_uint32 num_channels, t_double buffer[])
    t_error hil_read_encoder(t_card card, const t_uint32 encoder_channels[], t_uint32 num_channels, t_int32 buffer[])
    t_error hil_read_digital(t_card card, const t_uint32 digital_lines[], t_uint32 num_lines, t_boolean buffer[])
    t_error hil_read_other(t_card card, const t_uint32 other_channels[], t_uint32 num_channels, t_double buffer[])
    ...
    t_error hil_task_read_write(t_task task, t_uint32 num_samples,
