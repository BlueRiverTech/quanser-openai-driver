...
def print_state(currents, encoders, others):
    ...
    print("\t Gyro:       ({}, {}, {})".format(others[0], others[1], others[2]))
    print("\t Accel:      ({}, {}, {})".format(others[3],  others[4], others[5]))
...
def test_py_control():
    ...
    with QuanserAero(25) as a:
        ...
        while True:
            ...
            encoder2Deg = 1. * encoders[2] * (360.0 / 2048.0);  # pitch
            encoder3Deg = 1. * encoders[3] * (360.0 / 4096.0);  # yaw (encoder3 is higher resolution than the other encoders)
            print_state(currents, encoders, others)

            pitch_rad       = 0.01743 * encoder2Deg;
            yaw_rad         = 0.01743 * encoder3Deg;
            state_x[0]      = pitch_rad;
            state_x[1]      = yaw_rad;
